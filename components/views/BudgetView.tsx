...
export function BudgetView() {
  ...
  for (const e of entries) {
    ...
    actual[cat] = (actual[cat] ?? 0) + e.price;
  }

  const totalActual  = Object.values(actual).reduce((a, b) => a + b, 0);
  const totalPlanned = budgetCats.reduce((a, c) => a + c.planned, 0);

  const dailyTotals = days.map(d => {
    return entries.filter(e => e.start.startsWith(d.date)).reduce((s, e) => s + (e.price ?? 0), 0);
  });
  ...
  return (
    <div className="fade-in" style={{ maxWidth:680, display:'flex', flexDirection:'column', gap:20 }}>
      <div className="card" style={{ padding:24 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
          <div>
            ...
            <div className="serif" style={{ fontSize:36 }}>€{TRIP_META.budget.toLocaleString()}</div>
          </div>
          <div style={{ textAlign:'right' }}>
            <div className="muted" style={{ fontSize:11, letterSpacing:'.1em', textTransform:'uppercase', marginBottom:4 }}>Committed</div>
            <div className="mono" style={{ fontSize:22 }}>€{Math.round(totalActual).toLocaleString()}</div>
            <div className="muted" style={{ fontSize:12 }}>of €{totalPlanned.toLocaleString()} planned</div>
          </div>
        </div>
        <div style={{ height:8, background:'var(--paper-3)', borderRadius:4, overflow:'hidden', marginBottom:20 }}>
          <div style={{ height:'100%', width:`${Math.min(100,(totalActual/TRIP_META.budget)*100)}%`, background:'var(--accent)', borderRadius:4, transition:'width .4s' }}/>
        </div>
      ...
      {budgetCats.map(cat => {
        ...
        const pct     = Math.min(100, (spent / cat.planned) * 100);
        ...
        return (
          <div key={cat.id} className="card" style={{ padding:18 }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
              ...
              <div style={{ textAlign:'right' }}>
                <span className="mono" style={{ fontSize:15, color: over ? '#dc2626' : 'var(--ink)' }}>€{Math.round(spent).toLocaleString()}</span>
                <span className="muted" style={{ fontSize:12 }}> / €{cat.planned.toLocaleString()}</span>
              </div>
            </div>
